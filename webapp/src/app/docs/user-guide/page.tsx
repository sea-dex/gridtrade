...
Oneshot:          No`}</DocCodeBlock>
      ...
      {/* ── Understanding APY ── */}
      <DocH2 id="understanding-apy">Understanding APY</DocH2>
      <DocP>
        GridEx shows multiple APY views so you can separate overall grid performance from the part
        created by the grid strategy itself.
      </DocP>

      <DocH3>Real APY</DocH3>
      <DocP>
        <DocStrong>Real APY</DocStrong> is the annualized return of the grid as it actually ran.
        It includes the current value of the assets still held by the grid and any realized grid
        profit already earned.
      </DocP>
      <DocCodeBlock language="text">{`Real APY = (current_usd / init_usd) ** (8760 / elapsed_hours) - 1`}</DocCodeBlock>
      <DocUl>
        <DocLi><DocCode>init_usd</DocCode> — the USD value of the initial deposit when the grid was created.</DocLi>
        <DocLi><DocCode>current_usd</DocCode> — the USD value of the current grid position plus realized grid profit.</DocLi>
        <DocLi><DocCode>elapsed_hours</DocCode> — the runtime of the grid in hours.</DocLi>
      </DocUl>

      <DocH3>Grid APY</DocH3>
      <DocP>
        <DocStrong>Grid APY</DocStrong> measures the annualized excess return generated by the grid
        strategy itself. To compute it, GridEx first estimates a benchmark return for the same
        initial assets without grid execution.
      </DocP>
      <DocCodeBlock language="text">{`Benchmark APY = (tvl_usd / init_usd) ** (8760 / elapsed_hours) - 1
Grid APY = Real APY - Benchmark APY`}</DocCodeBlock>
      <DocUl>
        <DocLi><DocCode>tvl_usd</DocCode> — the current USD value of the benchmark portfolio without adding grid profit.</DocLi>
        <DocLi>A positive <DocStrong>Grid APY</DocStrong> means the grid outperformed passive holding over the same period.</DocLi>
        <DocLi>A negative <DocStrong>Grid APY</DocStrong> means passive holding would have done better over the same period.</DocLi>
      </DocUl>

      <DocH3>How to Read These Metrics</DocH3>
      <DocTable
        headers={['Metric', 'What It Tells You', 'Typical Use']}
        rows={[
          ['Real APY', 'Overall annualized performance of the live grid.', 'Check how the position is performing in total.'],
          ['Grid APY', 'Annualized excess return from grid execution versus passive holding.', 'Check whether the strategy itself is adding value.'],
        ]}
      />

      <DocBlockquote>
        APY is annualized from the grid&apos;s current runtime, so newer grids can show large swings.
        Use it as a directional metric, not a guaranteed forward yield.
      </DocBlockquote>
      ...
      <DocTable
        ...
        rows={[
          ...
          [
            ...
            'The annualized return metric used for a grid, including Real APY and Grid APY views.',
          ],
          [
            'Fee Tier',
